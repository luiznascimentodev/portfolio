...
interface Project {
  ...
  technologies: string[];
  techBadges?: string[]; // URLs das badges de tecnologia
  image: string;
  images: string[];
  github?: string;
  demo?: string;
...
export const Projects = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjectImage = async (
      ...
      // Tentar buscar imagens comuns de preview
      const previewPaths = [
        "main.png",
    ...
    const fetchReadme = async (
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Tentar extrair título específico
          const titleTagStart = "<!-- PROJECT_TITLE_START -->";
          const titleTagEnd = "<!-- PROJECT_TITLE_END -->";
          let projectTitle = "";

          if (readme.includes(titleTagStart) && readme.includes(titleTagEnd)) {
            const titleStartIndex =
              readme.indexOf(titleTagStart) + titleTagStart.length;
            const titleEndIndex = readme.indexOf(titleTagEnd);
            projectTitle = readme
              .substring(titleStartIndex, titleEndIndex)
              .trim();
          }

          // Tentar extrair tecnologias específicas
          const techTagStart = "<!-- PROJECT_TECH_START -->";
          const techTagEnd = "<!-- PROJECT_TECH_END -->";
          let projectTechnologies: string[] = [];
          let projectTechBadges: string[] = [];

          if (readme.includes(techTagStart) && readme.includes(techTagEnd)) {
            const techStartIndex =
              readme.indexOf(techTagStart) + techTagStart.length;
            const techEndIndex = readme.indexOf(techTagEnd);
            const techContent = readme
              .substring(techStartIndex, techEndIndex)
              .trim();

            // Extrair URLs de badges (img.shields.io)
            const badgeRegex =
              /!\[([^\]]+)\]\((https:\/\/img\.shields\.io\/badge\/[^)]+)\)/g;
            let badgeMatch;

            while ((badgeMatch = badgeRegex.exec(techContent)) !== null) {
              projectTechnologies.push(badgeMatch[1]); // Nome da tecnologia
              projectTechBadges.push(badgeMatch[2]); // URL da badge
            }

            // Fallback: se não encontrou badges, tentar extrair texto simples
          ...
          // Tentar extrair destaques específicos
          const highlightsTagStart = "<!-- PROJECT_HIGHLIGHTS_START -->";
          const highlightsTagEnd = "<!-- PROJECT_HIGHLIGHTS_END -->";
          let projectHighlights: string[] = [];

          if (
            readme.includes(highlightsTagStart) &&
            readme.includes(highlightsTagEnd)
          ) {
            const highlightsStartIndex =
              readme.indexOf(highlightsTagStart) + highlightsTagStart.length;
            const highlightsEndIndex = readme.indexOf(highlightsTagEnd);
            const highlightsContent = readme
              .substring(highlightsStartIndex, highlightsEndIndex)
              .trim();
            projectHighlights = highlightsContent
              .split("\n")
              .map((line) => line.trim())
              .filter((line) => line.length > 0);
          }

          // Tentar extrair live demo URL
          const demoTagStart = "<!-- PROJECT_DEMO_START -->";
          const demoTagEnd = "<!-- PROJECT_DEMO_END -->";
          let projectDemoUrl = "";

          if (readme.includes(demoTagStart) && readme.includes(demoTagEnd)) {
            const demoStartIndex =
              readme.indexOf(demoTagStart) + demoTagStart.length;
            const demoEndIndex = readme.indexOf(demoTagEnd);
            projectDemoUrl = readme
              .substring(demoStartIndex, demoEndIndex)
              .trim();
          }

          // Tentar extrair descrição entre tags específicas
          const tagStart = "<!-- PROJECT_TAG_START -->";
          const tagEnd = "<!-- PROJECT_TAG_END -->";

          let description = "";

          if (readme.includes(tagStart) && readme.includes(tagEnd)) {
            const startIndex = readme.indexOf(tagStart) + tagStart.length;
            const endIndex = readme.indexOf(tagEnd);
            const taggedContent = readme.substring(startIndex, endIndex);

            // Extrair apenas o parágrafo de descrição (remover headers, badges, links, etc)
            description = taggedContent
              .split("\n")
              .filter((line) => {
          ...
          // Fallback: pegar primeiros parágrafos limpos
          if (!description) {
            description = readme
              .split("\n")
              .filter((line) => {
    ...
    const fetchStarredRepos = async () => {
      try {
        ...
        const mappedProjectsPromises = repos.map(async (repo) => {
          ...
          // Tentar obter imagem customizada
          const customImage = await fetchProjectImage(
            repo.owner.login,
            repo.name
          );

          // Ordem de prioridade para imagem principal:
          // 1. Imagem customizada (main.png na raiz)
          // 2. Social preview do GitHub
          const mainImage =
            customImage ||
            `https://opengraph.githubassets.com/1/${repo.owner.login}/${repo.name}`;

          // Para o slider, usar apenas a imagem principal
